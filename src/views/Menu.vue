<template>
    ...
    <v-layout class="pt-4" v-if="userInfo && userInfo.type=='ADMIN'">
        ...
        <v-tabs-items v-model="tab" v-if="addedit==false ">
            ...
            <v-flex md10>
                <v-row>
                    <v-col md="4" lg="3" sm="6" xs="6" v-for="(item , i ) in Whole" :key="i">
                        <v-card class="blue-grey lighten-3">
                            <v-card-text>
                                <v-row no-gutters>
                                    ...
                                    <v-col cols="12" v-else-if="userInfo != null && userInfo.type !='ADMIN'">
                                          <v-btn small dark color="success" @click="transaction('order' , item )"><v-icon>mdi-receipt</v-icon>buy</v-btn>
                                        <v-btn small dark color="red" @click="transaction('cart' , item)"><v-icon small>mdi-cart</v-icon> cart</v-btn>
                                    </v-col>
                                    <v-col cols="12" v-if="userInfo != null && userInfo.type =='ADMIN'">
                                        <v-btn  @click="openAddEdit(item)" small dark color="success"><v-icon>mdi-pencil</v-icon>edit</v-btn>
                                        <v-btn  @click="deleteItem(item)" small dark color="red"><v-icon>mdi-trash</v-icon>delete</v-btn>
                                    </v-col>
                                </v-row>
            ...
            <v-tab-item>
                <center>
                    <v-flex md10>
                        <v-row>
                            <v-col md="4" lg="3" sm="6" xs="6" v-for="(item , i ) in Variety" :key="i">
                        <v-card class="blue-grey lighten-3" >
                            <v-card-text>
                                <v-row no-gutters>
                                    <v-col cols="12">
            ...
            <v-tab-item>
                <center>
                    <v-flex md10>
                        <v-row>
                            <v-col md="4" lg="3" sm="6" xs="6" v-for="(item , i ) in Dishes" :key="i">
                        <v-card class="blue-grey lighten-3">
                            <v-card-text>
                                <v-row no-gutters>
                                    <v-col cols="12">
        ...
        <v-container fluid v-else >
            <h2 class="text-center">{{editedIndex==-1 ? 'ADD NEW' : 'UPDATE'}} ITEM</h2>
                ...
                <v-flex md6 sm12 xs12 lg6 >
                    <v-row no-gutters>
                        <v-col cols="12">
                            ...
                            <v-text-field v-model="addObj.product_name" dark hide-details class="text1" outlined dense  type="email"></v-text-field>
                        </v-col>
                        <v-col cols="6">
                        ...
                        <v-col cols="6" v-if="editedIndex>-1">
                            <label>STATUS </label>
                            <v-autocomplete v-model="addObj.status" :items="Status" item-text="text" item-value="no" dark hide-details class="text1" outlined dense></v-autocomplete>
                         </v-col>
                        <v-col cols="12">
                            <label>IMAGE</label><br/>
                            <input type="file" @change="getFiles" name="file" id="file"  accept="image/png, image/jpeg"/>
                        </v-col>
                    ...
                    <v-layout>
                        <v-spacer/>
                        <v-btn @click="insertUpdateProduct()" small dark color="success">SAVE</v-btn>
                    </v-layout>
                </v-flex>
            </center>
            </v-container>
        </v-container>
        <v-dialog v-model="dialog" persistent scrollable max-width="800px"  transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar flat dense dark color="success">
                    <v-toolbar-title><v-icon>{{ type=='order' ? 'mdi-receipt-text-plus-outline'  : 'mdi-cart' }}</v-icon>{{ type=='order' ? 'BUY NOW' :'ADD TO CART' }}</v-toolbar-title>
                    <v-spacer/>
                    ...
                    <v-container fluid>
                        <v-row >
                            ...
                            <v-col>
                                ...
                                <span>
                                    <v-btn  x-small rounded @click="evalQuantity('-',obj)"><v-icon>mdi-minus</v-icon></v-btn>
                                    <input type="number" v-model="obj.quantity" style="width:15%;border:1px solid;text-align:center"/>
                                    <v-btn  x-small rounded @click="evalQuantity('+',obj)"><v-icon>mdi-plus</v-icon></v-btn>
                                </span>
                                <br/><br/>
                                <h2 v-if="type=='order'">Total : PHP {{ obj.total_price  }} </h2>
                                <v-btn  right v-if="type=='order'" dark color="success" @click="openTotalandDeilvery('buynow')"><v-icon>mdi-receipt</v-icon> BUY NOW </v-btn>
                                <v-btn  right v-else dark color="red" @click="addToCart()"><v-icon>mdi-cart</v-icon> ADD TO CART </v-btn>
                            </v-col>
                        </v-row>
        ...
        <TotalandDelivery :dialog="totalBuyandCart" :data="obj" :type="orderType" @closeDialog="closeDialog"/>
        </v-container>
    ...
    methods:{
        ...
        getFiles(e){
            ...
            if(files.length > 0 ){
                ...
                axios.post(`${this.api}uploadProducts`, formdata, {
                    headers: {
        ...
        deleteItem(item){
            if(confirm('Are you sure you want to delete this product? ')){
                axios.get(`${this.api}deleteProducts/${item.product_id}`).then(res=>{
                if(res.data){
        ...
        insertUpdateProduct(){
            ...
            if(this.editedIndex == -1 ){
                let index = this.Products.findIndex(x=> x.product_name.toUpperCase() == this.addObj.product_name.toUpperCase())
                if(index > -1 ){
