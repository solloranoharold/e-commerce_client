      ...
      <v-container fluid>
        ...
        <center v-if="userInfo.type=='USER'">
            ...
            <v-card>
               <v-toolbar flat dense>
                ...
                <v-spacer/>
                <a v-if="item.receipt && item.receipt!='null'" target="_blank" :href="`${filelister}receipt/${item.receipt}`">
                ...
                <v-tooltip bottom>
                    <template v-slot:activator="{ on, attrs }">
                        <v-icon color="indigo" @click="viewOrder(item)"  v-bind="attrs" v-on="on">mdi-eye</v-icon>
                    </template>
                    <span>View Order </span>
                </v-tooltip>
                <v-tooltip bottom v-if="item.orderStatus == 1">
                    <template v-slot:activator="{ on, attrs }">
                        <v-icon color="red" @click="updateStatus( item , 3)" v-bind="attrs" v-on="on">mdi-cancel</v-icon>
                    </template>
                    <span>Cancel</span>
                </v-tooltip>
                        ...
                        <tbody>
                            <tr v-for="(prod , x ) in item.Products" :key="x">
                             <td style="padding:0px;">
                                <v-img class="pr-5" max-width="200" :src="`${filelister}products/${prod.img_product}`" />
                            </td>
                            <td align="left">
                                {{ prod.product_name }}
                            </td>
        ...
        >
            ...
            <template v-slot:[`item.action`]="{item}">
                ...
                <v-tooltip bottom v-if="userInfo.fullname == 'ADMINISTRATOR'">
                    <template v-slot:activator="{ on, attrs }">
                        <v-icon color="orange" @click="updateStatus( item , 1)"  v-bind="attrs" v-on="on">mdi-account-clock</v-icon>
                    </template>
                    <span>Pending</span>
                </v-tooltip>
                <v-tooltip bottom v-if="userInfo.fullname == 'ADMINISTRATOR'">
                    <template v-slot:activator="{ on, attrs }">
                        <v-icon color="success" @click="updateStatus( item , 2)"  v-bind="attrs" v-on="on">mdi-truck</v-icon>
                    </template>
                    <span>For Delivery</span>
                </v-tooltip>
                <v-tooltip bottom v-if="userInfo.fullname == 'ADMINISTRATOR'">
                    <template v-slot:activator="{ on, attrs }">
                        <v-icon color="primary" @click="updateStatus( item , 4)" v-bind="attrs" v-on="on">mdi-check</v-icon>
                    </template>
                    <span>Delivered</span>
                </v-tooltip>
                <v-tooltip bottom v-if="item.orderStatus == 1">
                    ...
                    <v-simple-table dense>
                        ...
                        <tbody>
                            <tr v-for="(item , i ) in specificOrders" :key="i">
                                ...
                                <v-img class="pr-5" max-width="200" :src="`${filelister}products/${item.img_product}`" />
                                </td>
                                <td align="center"><h3>{{ item.product_name }}</h3></td>
                                <td align="center">{{ item.qty }}</td>
                                <td align="center">PHP : {{ (item.product_price * item.qty).toFixed(2) }}</td>
                            </tr>
                            <tr v-if="specificOrders.length > 0 ">
                                <td></td>
                                <td align="center">Shipping fee</td>
                                <td></td>
                                <td align="center">PHP : {{ specificOrders[0].shipping_fee.toFixed(2) }}</td>
                            </tr>
                            <tr v-if="specificOrders.length > 0 ">
                                <td></td>
                                <td></td>
                                <td></td>
                                <td style="font-weight:bold" align="center"> Total: {{ specificOrders[0].total_price.toFixed(2) }}</td>
                            </tr>
                        </tbody>
                    </v-simple-table>
                </v-card-text>
        ...
        <v-dialog v-model="uploadDialog" persistent scrollable max-width="300px">
            <v-card>
                ...
                <v-btn block dark color="indigo" @click="uploadReceipt()">UPLOAD</v-btn>
                </v-card-text>
            </v-card>
        </v-dialog>
    </v-container>
  ...
  methods:{
    ...
    saveDocument(){
        if(this.file){
            ...
            this.addObj.receipt  = moment().format('YYYYMMDDHHmmss')+'_'+this.file.name
